...
class FoodItemAPITests(APITestCase):
    ...
    def test_fuzzy_search_requires_authentication(self):
        response = self.client.get(reverse("food-item-fuzzy-search"), {"q": "chikn"})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_fuzzy_search_returns_similar_food_with_nutrition_fields(self):
        self.client.force_authenticate(self.user)

        response = self.client.get(reverse("food-item-fuzzy-search"), {"q": "chikn brest"})
        ...
        self.assertEqual(first["name"], "Chicken Breast")
        self.assertIn("id", first)
        self.assertIn("per_100g_kcal", first)
        self.assertIn("per_100g_protein", first)
        self.assertIn("per_100g_carbs", first)
        self.assertIn("per_100g_fat", first)

    def test_fuzzy_search_respects_limit(self):
        self.client.force_authenticate(self.user)

        response = self.client.get(reverse("food-item-fuzzy-search"), {"q": "r", "limit": 2})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertLessEqual(len(response.data["results"]), 2)

        response = self.client.get(reverse("food-item-fuzzy-search"), {"q": "ri", "limit": 2})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertLessEqual(len(response.data["results"]), 2)

    def test_fuzzy_search_returns_not_found_message_when_no_close_match(self):
        self.client.force_authenticate(self.user)

        response = self.client.get(reverse("food-item-fuzzy-search"), {"q": "zzzzzz"})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["message"], "No close matches found.")
