...
class MealLog(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.user.username}: {self.intake_date} {self.get_meal_type_display()} - {self.food_item.name}"
    ...
    def calculate_actual_nutrients(self) -> dict[str, Decimal]:
        ...
        return {
            "actual_kcal": self._round(factor * Decimal(self.food_item.per_100g_kcal)),
            "actual_protein": self._round(factor * Decimal(self.food_item.per_100g_protein)),
            "actual_carbs": self._round(factor * Decimal(self.food_item.per_100g_carbs)),
            "actual_fat": self._round(factor * Decimal(self.food_item.per_100g_fat)),
        }
